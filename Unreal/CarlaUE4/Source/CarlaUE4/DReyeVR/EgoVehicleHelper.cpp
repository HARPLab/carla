...
std::tuple<float, float> AEgoVehicle::GetAngles(FVector UnitGazeVec, FVector RotVec)
{
...
FVector AEgoVehicle::GenerateRotVec(FVector UnitGazeVec, float yawMax, float pitchMax, float vert_offset)
{
	...
	float pitch_dist_pos = tan(pitchMax + vert_offset);
	float pitch_dist_neg = tan(-pitchMax + vert_offset);
	float yaw_dist = tan(yawMax);
...
FVector AEgoVehicle::GenerateRotVecGivenAngles(FVector UnitGazeVec, float yaw, float pitch)
{
	// Normalize input vector
	UnitGazeVec = UnitGazeVec / UnitGazeVec.Size();
	float Unit_x0 = UnitGazeVec[0];
