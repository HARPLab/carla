...
void AEgoVehicle::ReadConfigVariables()
{
    ...
    // Peripheral targets
    ReadConfigValue("Peripheral", "TimeBetweenOnsetChunks", TimeBetweenFlash);
    ReadConfigValue("Peripheral", "TargetOnsetDuration", FlashDuration);
    ReadConfigValue("Peripheral", "TargetRadius", TargetRadius);
    ReadConfigValue("Peripheral", "TargetRenderDistance", TargetRenderDistance);
}
...
void AEgoVehicle::BeginPlay()
{
    ...
    // Initialize the lightball object
    InitLightBall();
...
void AEgoVehicle::InitLightBall()
{
    ...
    LightBallObject = World->SpawnActor<ALightBall>(FirstPersonCam->GetComponentLocation(), FRotator(0.0f, 0.0f, 0.0f),
                                                    LightBallSpawnInfo);
    LightBallObject->SetSize(TargetRadius); // Set the size from params
    LightBallObject->SetActorEnableCollision(false);
}

void AEgoVehicle::TickPeriphTarget(float DeltaSeconds)
{
    // Draw stimuli
    const FRotator &WorldRot = FirstPersonCam->GetComponentRotation();
    const FVector &WorldPos = FirstPersonCam->GetComponentLocation();
    ...
    GenerateSphere(HeadDirection, CombinedGazePosn, WorldRot, WorldPos, DeltaSeconds);
                                 ...
                                 const FRotator &WorldRot, const FVector &CombinedOriginIn, float DeltaTime)
{
    ...
    if (EgoSensor->IsReplaying())
    {
        ...
        LightBallObject->SetLocation(FirstPersonCam->GetComponentLocation() + RotVecDirection * TargetRenderDistance);
        if (Replay->GetUserInputs().LightOn)
...
FVector AEgoVehicle::GenerateRotVecGivenAngles(FVector UnitGazeVec, float yaw, float pitch) const
{
    // Normalize input vector
    UnitGazeVec = UnitGazeVec / UnitGazeVec.Size();
    float Unit_x0 = UnitGazeVec[0];
